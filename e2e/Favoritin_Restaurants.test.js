...
Scenario('favoriting one restaurant', async ({ I }) => {
  ...
  const firstRestoTitle = (await I.grabTextFrom(firstResto)).trim(); // Gunakan .trim()
  I.click(firstResto);
  I.wait(3);
  ...
  const favoritedRestoTitle = (await I.grabTextFrom('#resto-title')).trim(); // Gunakan .trim()
...
Scenario('unfavoriting one restaurant', async ({ I }) => {
  ...
  const favoritedRestoTitle = (await I.grabTextFrom(firstRestolike)).trim(); // Gunakan .trim()
  assert.strictEqual(firstRestoTitle, favoritedRestoTitle);
  ...
  const onFav = (await I.grabTextFrom('#resto-item__not__found')).trim(); // Gunakan .trim()
